<?php
    ...
    $sql = "SELECT * FROM segundo_año as sg INNER JOIN alumnos as al ON sg.alumno_id = al.alumno_id INNER JOIN aulas as au ON sg.aula_id = au.aula_id INNER JOIN periodos as pe ON sg.periodo_id = pe.periodo_id INNER JOIN menciones as me ON sg.mencion_id = me.mencion_id WHERE sg.statussg != 0 AND sg.aula_id = $seccion ORDER BY al.cedulaes";
                    ...
                    ?>
                      <tr>
                        ...
                        if ($privilegios == 1){
                            ...
                            <a href="lista_notas.php?seccion='.$seccion.'&curso=2&cursante='.$data['alumno_id'].'&cedu='.$data['cedulaes'].'&nombrestu='.$data['nombre_alumno'].'&apellistu='.$data['apellido_alumno'].'&mencion='.$data['mencion_id'].'&mencion_nomb='.$data['mencion_nombre'].'&nacionstu='.$data['nacionalidad'].'&fecnac='.$nacimientofe.'&feces='.$data['nombre_periodo'].'&nomsecci='.$nombreSeccion.'&numelis='.$numerolista.'&elPerio='.$data['periodo_id'].'" class="btn btn-info btn-sm">Ver Notas</a>
                                                ';
