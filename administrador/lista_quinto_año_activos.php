<?php
    ...
    $sql = "SELECT * FROM quinto_año as sg INNER JOIN grupos as gp ON sg.grupo_id = gp.grupo_id INNER JOIN alumnos as al ON sg.alumno_id = al.alumno_id INNER JOIN aulas as au ON sg.aula_id = au.aula_id INNER JOIN periodos as pe ON sg.periodo_id = pe.periodo_id INNER JOIN menciones as me ON sg.mencion_id = me.mencion_id WHERE sg.statusqn != 0 AND sg.aula_id = $seccion AND pe.estado = 1 ORDER BY al.cedulaes";
...
<main class="app-content">
      <div class="app-title">
        <div>
        <h1><i class="fa fa-dashboard"></i> Lista de Alumnos de Quinto Año Seccion <?php echo $nombreSeccion?></h1>
            <button class="btn btn-success" type="button" onclick="openModalQuinto()">Nuevo Registro</button>
            <a href="seleccion_notas.php?curso=5" class="btn btn-info">Generar Notas</a>
            <a href="nomina_estudiantil.php?curso=5&aul=<?php echo $nombreSeccion;?>&aulid=<?php echo $seccion;?>" class="btn btn-info">Generar Nomina Estudiantil</a>
        </div>
