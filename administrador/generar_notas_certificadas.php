      ...
      <div class="modal-body">
        ...
        <form id="generar_notas_certificadas_reporte" name="generar_notas_certificadas_reporte" action="generar_notas_certificadas_reporte.php" method="POST">
            ...
            <?php 
              ...
              if ($anio < 5) {
                for ($k=$anio+1; $k <= 5 ; $k++) { ?>
                  <input type="hidden" class="form-control" name="cantidadM_<?= $k?>" id="cantidadM_<?= $k?>" value="<?php echo $cantidadMaterias[1];?>" readonly>
                  <input type="hidden" class="form-control" name="grupo_<?= $k?>" id="grupo_<?= $k?>" value="<?php echo '*****';?>" readonly>
                  <?php
                  for ($l=0; $l < $cantidadMaterias[$k]; $l++) { 
                    
                  ?>
                  
                        <input type="hidden" class="form-control" name="nombreM_<?= $k?>_<?= $l?>" id="nombreM_<?= $k?>_<?= $l?>" value="**********************" readonly>
                        <input type="hidden" class="form-control" name="nombreN_<?= $k?>_<?= $l?>" id="nombreN_<?= $k?>_<?= $l?>" value="**************" readonly>
                        <input type="hidden" class="form-control" name="periodoN_<?= $k?>_<?= $l?>" id="periodoN_<?= $k?>_<?= $l?>" value="****" readonly>
                        <input type="hidden" class="form-control" name="siglasM_<?= $k?>_<?= $l?>" id="siglasM_<?= $k?>_<?= $l?>" value="<?php echo $siglas[$k][$l];?>" readonly>              
                        <input type="hidden" class="form-control" name="promedio_<?= $k?>_<?= $l?>" id="promedio_<?= $k?>_<?= $l?>" value="****" readonly>              
                        <input type="hidden" class="form-control" name="tipoN_<?= $k?>_<?= $l?>" id="tipoN_<?= $k?>_<?= $l?>" value="****" readonly>              
