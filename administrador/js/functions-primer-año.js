document.addEventListener('DOMContentLoaded',function(){
    ...
    formPrimerAño.onsubmit = function(e) {
        ...
        var url = window.location.origin +'/liceoEjido/administrador/models/primer-año/ajax-primer-año.php';
...
function showAlumno() {
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var url = window.location.origin +'/liceoEjido/administrador/models/options/options-alumno.php';
...
function showGrupos() {
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var url = './models/options/options-grupos.php';
    request.open('GET',url,true);
    request.send();
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            var data = JSON.parse(request.responseText);
            data.forEach(function(valor){
                data  += '<option value="'+valor.grupo_id+'">'+valor.nombre_grupo+'</option>';
            });
            document.querySelector('#listGrupos').innerHTML = data;
        }
    }
}
        ...
        request.onreadystatechange = function() {
            if(request.readyState == 4 && request.status == 200) {
                ...
                if(data.status) {
                    ...
                    document.querySelector('#listMencion').value = data.data.mencion_id;
