<?php
    ...
    $sql = "SELECT * FROM primer_año as pa INNER JOIN alumnos as al ON pa.alumno_id = al.alumno_id INNER JOIN aulas as au ON pa.aula_id = au.aula_id INNER JOIN periodos as pe ON pa.periodo_id = pe.periodo_id INNER JOIN menciones as me ON pa.mencion_id = me.mencion_id  WHERE pa.statuspr != 0 AND pa.aula_id = $seccion ORDER BY al.cedulaes";
                    ...
                    ?>
                      <tr>
                        ...
                        if ($privilegios == 1){
                            ...
                            <a href="lista_notas.php?seccion='.$seccion.'&curso=1&cursante='.$data['alumno_id'].'&cedu='.$data['cedulaes'].'&nombrestu='.$data['nombre_alumno'].'&apellistu='.$data['apellido_alumno'].'&mencion='.$data['mencion_id'].'&mencion_nomb='.$data['mencion_nombre'].'&nacionstu='.$data['nacionalidad'].'&fecnac='.$nacimientofe.'&feces='.$data['nombre_periodo'].'&nomsecci='.$nombreSeccion.'&numelis='.$numerolista.'&elPerio='.$data['periodo_id'].'" class="btn btn-info btn-sm">Ver Notas</a>
                                                ';
